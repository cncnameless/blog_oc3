...
class ControllerCatalogAuthor extends Controller {
    ...
    protected function getList() {
        ...
        $data['sort_status'] = $this->url->link('catalog/author', 'user_token=' . $this->session->data['user_token'] . '&sort=status' . $url, true);
    ...
    protected function getForm() {
        ...
        // Исправление: создаем массив магазинов без дублирования
        $data['stores'] = array();
        
        // Добавляем магазин по умолчанию
        $data['stores'][] = array(
            'store_id' => 0,
            'name'     => $this->language->get('text_default')
        );
        
        // Добавляем остальные магазины (исключая дубликат магазина по умолчанию)
        $stores = $this->model_setting_store->getStores();
        foreach ($stores as $store) {
            // Проверяем, что store_id не равен 0 (чтобы избежать дублирования)
            if ($store['store_id'] != 0) {
                $data['stores'][] = array(
