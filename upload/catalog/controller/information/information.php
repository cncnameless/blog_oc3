...
class ControllerInformationInformation extends Controller {
    public function index() {
        ...
        if ($information_info) {
            ...
            // Получаем категории блога для статьи
            ...
            if ($blog_categories && isset($this->request->get['blog_path'])) {
                // Статья из блога - строим полный путь категорий
                $data['breadcrumbs'][] = array(
                ...
                // Строим путь категорий для хлебных крошек
                ...
                // Статья из блога, но blog_path не передан - используем первую категорию
                $data['breadcrumbs'][] = array(
                    'text' => $this->language->get('text_blog'),
                    'href' => $this->url->link('information/blog_category')
                );
            ...
            // Убрали блок else - для статей без категорий блога не добавляем "Информация"

            // Текущая статья (без ссылки)
            $data['breadcrumbs'][] = array(
            ...
            // Показываем категории блога для статьи
            ...
            foreach ($blog_categories as $blog_category_id) {
                ...
                if ($blog_category_info) {
                    $data['blog_categories'][] = array(
                        ...
                        'href' => $this->url->link('information/blog_category', 'blog_path=' . $blog_category_id)
                    );
                }
            }
            ...
            // === ДОБАВЛЯЕМ ПОЛУЧЕНИЕ АВТОРОВ СТАТЬИ ===
            $data['authors'] = array();
            ...
            // === ДОБАВЛЯЕМ MICRODATA ДЛЯ SCHEMA.ORG ===
